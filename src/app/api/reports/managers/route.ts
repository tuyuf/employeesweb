...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Execute count query using unsafeQueryRaw for dynamic SQL
    ...
    if (!status || status === 'Current') {
      ...
      // Add status condition if not already filtering by it
      if (status !== 'Current') {
    ...
    console.error('Manager reports API error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch manager profiles' },
      { status: 500 }
    );
  }
}
